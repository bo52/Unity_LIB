...
namespace mesh2023
{
    static public class stCube256
    {
        static public void Куб(this IMesh m, Vector3 КоординатаБлока, byte КодБлока)
        {
            if (КодБлока == 0) return;
            m.Вертексы.Центр = КоординатаБлока;
            //треугольники
            foreach (var ИндексУникальногоТреугольника in УникальныеТриангуляционныеБлоки[КодБлока])
            {
                //треугольник
                for (byte i = 0; i < 3; i++)
                {
                    m.Вертексы.Строим(stCube256_adges.ВекторТреугольника(ИндексУникальногоТреугольника, i),
                        () =>
                        {
                            return new Vector2[8]
                            {
                            Vector2.zero,
                            new Vector2(КоординатаБлока.x,КоординатаБлока.y),//uv2 Координата блока 
                            new Vector2(КоординатаБлока.z,КодБлока),//uv3 Координата блока и КодБлока
                            Vector2.zero,
                            Vector2.zero,
                            Vector2.zero,
                            Vector2.zero,
